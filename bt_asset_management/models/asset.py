...
class BtAsset(models.Model):
    ...
    def _compute_economical_asset_amount(self):
        for asset in self:
            if asset.economical_asset_id:
                if asset.economical_asset_id.depreciation_move_ids:
                    closets_date_move_id = self.env['account.move'].search([('state', 'not in', ('posted', 'cancel')), (
                        'id', 'in', asset.economical_asset_id.depreciation_move_ids.ids), ('date', '>=', date.today())],
                                                                           order='date asc', limit=1)
                    asset.economical_asset_amount = closets_date_move_id and closets_date_move_id.asset_remaining_value or 0.0
                else:
